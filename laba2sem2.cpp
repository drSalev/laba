...
int Search(char* pole, char* string);
void MenuSearch(struct data* array, int n, int flug);
			...
			do
			{
				...
				case 1:
					do
					{
						...
						{
						case 1:
							MenuSearch(students, n, 1);
							break;
						case 2:
							MenuSearch(students, n, 2);
							break;
						case 3:
							MenuSearch(students, n, 3);
							break;
						case 4:
							a++;
							break;
	...
	for (i = 1; i < n; i++)
	{
		...
		do
		{
			...
			case 7:
				...
				word[a + 1] = '\0';
				array[i - 1].med.top_pressure[2] = atoi(word);
				break;
			case 8:
				for (b = S; b < E; b++)
				{
				...
				word[a + 1] = '\0';
				array[i - 1].med.bottom_pressure[3] = atoi(word);
				break;
			}
		} while (cmdchar[i][j] != '\0');
	}
...
void Input(struct data* array, int i)
{
	...
	{
		while (1) {
			printf(" Введите верхнее давление %d студента: ", i + 1);
			if (scanf_s(" %d", &array[i].med.top_pressure[2])) {
				break;
			}
			else {
				printf(" Введите число.\n ");
		...
		while (1) {
			printf(" Введите нижнее давление %d студента: ", i + 1);
			if (scanf_s(" %d", &array[i].med.bottom_pressure[3])) {
				break;
			}
			else {
				printf(" Введите число.\n ");
...
void Output(struct data* array, int i)
{
	...
	if (array[i].option == 2)
	{
		printf(" Верхнее давление %d студента: %d\n", i + 1, array[i].med.top_pressure[2]);
		printf(" Нижнее давление %d студента: %d\n", i + 1, array[i].med.bottom_pressure[3]);
	}
	printf("\n");
}
int Delete(struct data* array, int n, int flug)
{
	...
	for (int i = n - 1; i >= 0; i--)
	{
		if (a[i] >= 0)
		{
			for (int j = a[i]; j < n; j++)
			{
				array[j].option = array[j + 1].option;
				...
				if (array[j].option == 2)
				{
					array[j].med.top_pressure[2] = array[j + 1].med.top_pressure[2];
					array[j].med.bottom_pressure[3] = array[j + 1].med.bottom_pressure[3];
				}
			}
			e--;
		}
...
void MenuSearch(data* array, int n, int flug) {
	int i, k, j = 0, a = 0;
	char* string;
	...
	while ((string[j++] = getchar()) != '\n')
		string = (char*)realloc(string, j + 1);
	string[j - 1] = '\0';
	switch (flug) {
	case 1:
		for (i = 0; i < n; i++) {
			k = Search(array[i].fio.surname, string);
			if (k == 0)
			{
				Output(array, i);
				a++;
	...
	case 2:
		for (i = 0; i < n; i++) {
			k = Search(array[i].fio.name, string);
			if (k == 0)
			{
				Output(array, i);
				a++;
	...
	case 3:
		for (i = 0; i < n; i++) {
			k = Search(array[i].fio.partonymic, string);
			if (k == 0)
			{
				Output(array, i);
				a++;
